...
function installBlindbox(app, dataDir, options = {}) {
  const { createPostgresStorage, transactionalRoutes } = require('./blindbox-storage');
  const store = options.store || (process.env.BLINDBOX_DATABASE_URL ? createPostgresStorage(process.env.BLINDBOX_DATABASE_URL) : null);
  const release = require('./blindbox-release').installRelease(app, store);
  const accounts = require('./blindbox-accounts').installAccounts(app, dataDir, store);
  const keyPath = path.join(dataDir, 'blindbox-signing-key');
  try { if (!store) fs.writeFileSync(keyPath, randomBytes(32), { flag: 'wx', mode: 0o600 }); }
  catch (error) { if (error.code !== 'EEXIST') throw error; }
  const key = process.env.BLINDBOX_SIGNING_KEY || (!store && fs.readFileSync(keyPath));
  const sign = data => createHmac('sha256', key || store.key).update(data).digest('base64url');
  ...
  const gifts=installGiftReceipts(app,dataDir,sign,decode,store);
  ...
  transactionalRoutes(app,store,['accounts']).post('/api/blindbox/draw', (req, res) => {
    if (release.production && !release.persistent && !store) return res.status(503).json({error:'持久化存储尚未配置，暂时无法创建礼物。'});
  ...
  transactionalRoutes(app,store,['receipts']).get('/api/blindbox/reveal/:token', (req, res) => {
    try {
