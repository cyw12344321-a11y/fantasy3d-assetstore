...
function releaseConfig(env = { BLINDBOX_SUPPORT_EMAIL: '715341216@qq.com', ...process.env }) {
  ...
  const origin = publicOrigin(env.BLINDBOX_PUBLIC_ORIGIN || (env.RENDER === 'true' ? env.RENDER_EXTERNAL_URL : undefined));
...
function installRelease(app, store) {
  const config = releaseConfig();
  app.get('/api/blindbox/config', async (req,res) => {
    let databaseReady = false;
    if (store) { try { await store.ready(); databaseReady = true; } catch {} }
    res.set('Cache-Control','no-store').json({
    sharingEnabled: store ? databaseReady && config.production && Boolean(config.publicOrigin) : config.sharingEnabled, publicOrigin: config.publicOrigin,
