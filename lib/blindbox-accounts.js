...
function installAccounts(app, dataDir) {
  // Ephemeral hosting must not silently accept accounts that disappear on deploy.
  if ((process.env.RENDER || process.env.NODE_ENV === 'production') && process.env.BLINDBOX_PERSISTENT_STORAGE !== 'true') {
  ...
  function session(req) {
    ...
    const record = data.sessions.find(s => s.hash === digest(token) && s.expires > Date.now());
    return record ? data.users.find(u => u.id === record.userId) || null : null;
  }
  ...
  app.post('/api/blindbox/account/:action', mutation, (req, res) => {
    ...
    if (action === 'delete') {
      ...
      save({...data,users:data.users.filter(u=>u.id!==user.id),sessions:data.sessions.filter(s=>s.userId!==user.id)});
      setCookie(req,res,'',0);return res.json({user:null,deleted:true});
    }
    if (action === 'logout') {
      const token = (req.headers.cookie || '').match(/(?:^|;\s*)f3d_session=([a-f0-9]{64})(?:;|$)/)?.[1];
      if (token) save({ ...data, sessions: data.sessions.filter(s => s.hash !== digest(token)) });
    ...
    const existing = data.users.find(u => u.username === normalized);
  ...
  return {
    addDraw(req, characterId) {
      ...
      const next = structuredClone(data), user = next.users.find(u => u.id === current.id);
