...
function installGiftReceipts(app, dataDir, sign, decode, store) {
  app = require('./blindbox-storage').transactionalRoutes(app, store, ['receipts']);
  const file = path.join(dataDir, 'blindbox-gift-receipts.json');
  let records = !store && fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8')) : {};
  const read = () => store ? store.get('receipts') : records;
  ...
  const revoked = token => Boolean(read()[hash(token)]?.revoked);
  function update(key, record) {
    const cutoff=Date.now()-31*86400000;
    const next=Object.fromEntries(Object.entries(read()).filter(([,v])=>v.createdAt>cutoff));
    if (!next[key] && Object.keys(next).length>=5000) throw Error('礼物回执暂满，请稍后再试。');
    next[key]=record;
    if (store) return store.set('receipts', next);
    fs.writeFileSync(file+'.tmp',JSON.stringify(next),{mode:0o600});fs.renameSync(file+'.tmp',file);records=next;
  }
  ...
  app.get('/api/blindbox/gifts/:token', (req,res)=>{
    ...
    const record=read()[hash(token)];const votes=Object.values(record?.votes||{});
  ...
  app.post('/api/blindbox/gifts/:token/:action',(req,res)=>{
    ...
    const key=hash(token),record=structuredClone(read()[key]||{createdAt:result.createdAt,votes:{}});
