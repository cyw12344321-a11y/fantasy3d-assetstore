...
export function setupGifting(api, reveal) {
  ...
  const personal=document.createElement('label');personal.className='personal-note';personal.textContent=choose('想亲口说的一句话（可选）','One line in your own words (optional)');const note=document.createElement('textarea');note.id='gift-note';note.maxLength=60;note.rows=2;note.placeholder=choose('昵称和留言会出现在分享链接中，请勿填隐私信息。','Use nicknames. Anyone with the link can see your message.');personal.append(note);form.insertBefore(personal,form.querySelector('fieldset'));
