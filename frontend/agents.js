(() => {
  ...
  async function doAcquire() {
    try {
      ...
      const msg = `🎣 站内线索检查完成\n\n来源：${a.source}\n真实线索：${a.leadsGenerated} 人\n真实成交：${a.converted} 人\n\n${a.action}`;
      if (currentAgent && (currentAgent.id === "support" || currentAgent.id === "manager")) {
        appendMessage("agent", msg, new Date().toISOString());
      } else {
        alert("站内线索检查完成，当前没有可验证的新客户或成交。");
      }
      addLog("🎣 线索检查", `${a.source}：${a.leadsGenerated} 条真实线索，${a.converted} 笔真实成交`);
    } catch (err) {
